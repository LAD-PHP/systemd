...
*RestartSec=60s* - позволяет указать время простоя в секундах, после которого systemd перезапускает службу после аварии (по умолчанию 5 секунд).  
...
•	так же тут отображается путь к файлю модуля\сервиса и включён или нет сервис в автозагрузку
3.	Состояние active(running) – сервис активен и запущен. Другие возможные состояния:
...
•	Так же здесь можно увидеть с какого времени служба запущена
